...
// Copyright (c) 2014 The Keplercoin developers
...
const string strMessageMagic = "Keplercoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    int64 nSubsidy = nStartSubsidy;
	    ...
	    if(nHeight == 1) { nSubsidy = 2300000 * COIN; }    // 2,300,000 KPL                  //PREMINE
    else if(nHeight <= 60 && nHeight > 1) { nSubsidy = 0 * COIN; } // 0.0 KPL
	else if(nHeight <= 600 && nHeight > 60) { nSubsidy = 300 * COIN; } // 162,000 KPL
	else if(nHeight <= 1000 && nHeight > 600) { nSubsidy = 200 * COIN; } // 80,000 KPL
	else if(nHeight <= 2000 && nHeight > 1000) { nSubsidy = 100 * COIN; }  // 100,000 KPL
	// max 2,642,000
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KeplercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KeplercoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("Keplercoin miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoin-miner");
            ...
            {
                printf("Keplercoin miner terminated\n");
                throw;
            }
